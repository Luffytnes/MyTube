  ...
  if (!playlist) {
    return (
      <div className="flex flex-col items-center justify-center py-32 min-h-screen">
        ...
        <p className="text-yt-text-muted">Playlist introuvable.</p>
      </div>
    )
  }
    ...
    <div className="px-4 py-6 max-w-4xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row gap-6 mb-8">
        ...
        <div className="flex flex-col justify-end min-w-0">
          <p className="text-yt-text-muted text-xs uppercase tracking-widest mb-1">Playlist</p>
          <h1 className="text-yt-text text-3xl font-bold mb-2">{playlist.name}</h1>
          <p className="text-yt-text-muted text-sm">{playlist.tracks.length} titre{playlist.tracks.length !== 1 ? 's' : ''}</p>
      ...
      {playlist.tracks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-yt-text-muted text-sm">Aucun titre dans cette playlist.</p>
          <p className="text-yt-text-muted text-xs mt-1">Ajoute des titres depuis la recherche ou les pages artistes.</p>
        </div>
      ) : (
